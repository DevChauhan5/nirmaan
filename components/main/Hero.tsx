...
const Hero = () => {
  useGSAP(()=>{
      ...
      gsap.fromTo('#hero-text, #hero-desc',{
        opacity:0,
      },{
        opacity:1,
        delay:1.5,
      ...
      gsap.fromTo('#hero-btn1, #hero-btn2',{
        opacity:0,
        y:300,
      },{
        opacity:1,
  ...
  return (
    <section className="min-h-screen overflow-hidden mt-2 mx-4 flex flex-col justify-center items-center border-b py-12 -translate-y-4 rounded-lg relative object-contain">
      ...
      <p id="hero-desc" className="opacity-0 text-xl mb-10 uppercase md:text-4xl lg:text-5xl font-extrabold bg-clip-text text-transparent bg-[linear-gradient(to_right,theme(colors.yellow.400),theme(colors.yellow.300),theme(colors.orange.200),theme(colors.yellow.300),theme(colors.yellow.400))] bg-[length:200%_auto] animate-gradient">
        <TypeAnimation
          sequence={[
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <Button id="hero-btn1" size={"lg"} className="text-xl opacity-0" asChild>
          <Link href={"https://forms.gle/d4FyyGoQ7oRrAn3z7"}>Register Now</Link>
        </Button>
        <Button id="hero-btn2" size={"lg"} variant={"secondary"} className="text-xl opacity-0">
          View Schedule
        </Button>
      </div>
    </section>
